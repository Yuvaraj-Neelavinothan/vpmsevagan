                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ $id_service ? 'Update ' : 'Add new ' }}service
                            </h3>
                            ...
                            <div class="grid gap-4 mb-4 sm:grid-cols-3">
                                ...
                                <div>
                                    <label for="service_duration"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Service
                                        Duration(min)</label>
                                    <input type="number" wire:model="service_duration" min="0"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="30">
                                </div>
                                <div>
                                ...
                                <div>
                                    <label for="dropzone-file"
                                        ...
                                        @if ($service_image)
                                            @if (is_string($service_image))
                                                <img class="rounded-lg"
                                                    src="{{ asset('upload_img/' . $service_image) }}">
                                            @else
                                                <img class="rounded-lg" src="{{ $service_image->temporaryUrl() }}">
                                            @endif
                                        @else
                                            <div wire:loading wire:target="service_image">
                                            ...
                                            <div wire:loading.remove wire:target="service_image"
                                        ...
                                        @endif
                                        <input wire:model="service_image" id="dropzone-file" type="file"
                                            class="hidden" />
                                    </label>
                                    @error('service_image')
                                        <span class="error text-red-300">{{ $message }}</span>
        ...
        <div class="mx-auto max-w-screen-2xl">
            <div class="relative overflow-x-auto shadow-md dark:bg-gray-800 sm:rounded-lg">
                <table class="min-w-[1300px] text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    @php
                        $page = $hk_services->currentPage();
                        $per_page = $hk_services->perPage();
                    ...
                    <tbody>
                        @forelse ($hk_services as $hk)
                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-2">{{ $hk->service_code }}</td>
                                <th scope="row"
                                    class="flex items-center px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <img src="{{ asset('upload_img/' . $hk->service_image) }}" alt=""
                                        class="w-auto h-7 mr-3 rounded-lg">
                                    {{ $hk->service_name }}
                                </th>
                                <td class="px-4 py-2">{{ $hk->service_description }}
                                </td>
                                ...
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="flex items-center">
                                        ...
                                        <span class="ml-1 text-gray-500 dark:text-gray-400">{{ $hk->rating }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $hk->service_duration }}
                                ...
                                <td class="px-4 py-2"><label class="items-center mb-5 cursor-pointer">
                                        <input wire:click="toggle_status({{ $hk->id }})" type="checkbox"
                                            class="sr-only peer" {{ $hk->service_status == 1 ? 'checked' : '' }}>
                                        <div
