<div>
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ $id_others ? 'Update ' : 'Add new ' }}other service
                            </h3>
                            ...
                            <div class="grid gap-4 mb-4 sm:grid-cols-3">
                                <div>
                                    <label for="code"
                                        ...
                                        Service
                                        Code</label>
                                    <input type="text" wire:model="others_code"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Type others code">
                                    @error('others_code')
                                ...
                                <div>
                                    <label for="name"
                                        ...
                                        Service
                                        Name</label>
                                    <input type="text" wire:model="others_name"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Type others name">
                                    @error('others_name')
                                        <span class="error text-red-300">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div>
                                    <label for="m_price"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Marked
                                        Price</label>
                                    <input type="number" wire:model.live="marked_price" min="0"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="299">
                                    @error('marked_price')
                                        <span class="error text-red-300">{{ $message }}</span>
                                ...
                                <div>
                                    <label for="area"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Area(feet)</label>
                                    <input type="text" wire:model="area"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="1X1">
                                </div>
                                <div>
                                    <label for="others_duration"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Other
                                        Service
                                        Duration(min)</label>
                                    <input type="number" wire:model="others_duration" min="0"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="30">
                                </div>
                                <div>
                                    <label for="description"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">others
                                        Description</label>
                                    <textarea wire:model="others_description" rows="4"
                                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Write others description here"></textarea>
                                </div>
                                ...
                                <div>
                                    <label for="dropzone-file"
                                        ...
                                        @if ($others_image)
                                            @if (is_string($others_image))
                                                <img class="rounded-lg"
                                                    src="{{ asset('upload_img/' . $others_image) }}">
                                            @else
                                                <img class="rounded-lg" src="{{ $others_image->temporaryUrl() }}">
                                            @endif
                                        @else
                                            <div wire:loading wire:target="others_image">
                                            ...
                                            <div wire:loading.remove wire:target="others_image"
                                        ...
                                        @endif
                                        <input wire:model="others_image" id="dropzone-file" type="file"
                                            class="hidden" />
                                    </label>
                                    @error('others_image')
                                        <span class="error text-red-300">{{ $message }}</span>
    ...
    @include('status-msg')
    {{-- search,export,record per page and add others --}}
                ...
                <table class="min-w-[1300px] text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr class="bg-gray-200">
                            ...
                            <th scope="col" class="px-4 py-3">other service name</th>
                    ...
                    @php
                        $page = $other_services->currentPage();
                        $per_page = $other_services->perPage();
                        $sno = ($page - 1) * $per_page + 1;
                    @endphp
                    <tbody>
                        @forelse ($other_services as $o_service)
                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                <td class="w-4 px-4 py-3">
                                    {{ $sno++ }}
                                </td>
                                <td class="px-4 py-2">{{ $o_service->others_code }}</td>
                                <th scope="row"
                                    class="flex items-center px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <img src="{{ asset('upload_img/' . $o_service->others_image) }}" alt=""
                                        class="w-auto h-7 mr-3 rounded-lg">
                                    {{ $o_service->others_name }}
                                </th>
                                <td class="px-4 py-2">{{ $o_service->others_description }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $o_service->marked_price }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $o_service->offer_percentage }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $o_service->offer_price }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $o_service->original_price }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="flex items-center">
                                        ...
                                        <span class="ml-1 text-gray-500 dark:text-gray-400">{{ $o_service->rating }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $o_service->others_duration }}
                                </td>
                                <td class="px-4 py-2"><label class="items-center mb-5 cursor-pointer">
                                        <input wire:click="toggle_status({{ $o_service->id }})" type="checkbox"
                                            class="sr-only peer" {{ $o_service->others_status == 1 ? 'checked' : '' }}>
                                        <div
                                ...
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div x-data="{ open: false }" class="relative">
                                        ...
                                        <div x-show="open" @click.away="open = false" x-transition
                                            ...
                                            <a wire:click="edit({{ $o_service->id }})"
                                            ...
                                            <a wire:click="delete({{ $o_service->id }})"
                                                wire:confirm="Are you sure you want to delete this others Label?"
                                                class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"><svg
