<div>
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <form wire:submit="save">
                            <div class="grid gap-4 mb-4 sm:grid-cols-3">
                                <div>
                                    <label for="code"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Wash
                                        Code</label>
                                    <input type="text" wire:model="wash_code"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Type wash code">
                                    @error('wash_code')
                                ...
                                <div>
                                    <label for="name"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Wash
                                        Name</label>
                                    <input type="text" wire:model="wash_name"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Type wash name">
                                    @error('wash_name')
                                        <span class="error text-red-300">{{ $message }}</span>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    <label for="wash_duration"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Wash
                                        Duration(min)</label>
                                    <input type="number" wire:model="wash_duration" min="0"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="30">
                                </div>
                                <div>
                                    <label for="description"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Wash
                                        Description</label>
                                    <textarea wire:model="wash_description" rows="4"
                                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                        placeholder="Write wash description here"></textarea>
                                </div>
                                ...
                                <div>
                                    <label for="dropzone-file"
                                        ...
                                        @if ($wash_image)
                                            @if (is_string($wash_image))
                                                <img class="rounded-lg" src="{{ asset('upload_img/' . $wash_image) }}">
                                            @else
                                                <img class="rounded-lg" src="{{ $wash_image->temporaryUrl() }}">
                                            @endif
                                        @else
                                            <div wire:loading wire:target="wash_image">
                                            ...
                                            <div wire:loading.remove wire:target="wash_image"
                                        ...
                                        @endif
                                        <input wire:model="wash_image" id="dropzone-file" type="file"
                                            class="hidden" />
                                    </label>
                                    @error('wash_image')
                                        <span class="error text-red-300">{{ $message }}</span>
    ...
    @include('status-msg')
    {{-- search,export,record per page and add wash --}}
                ...
                <table class="min-w-[1300px] text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr class="bg-gray-200">
                            ...
                            <th scope="col" class="px-4 py-3">wash name</th>
                    ...
                    <tbody>
                        @forelse ($washes as $wash)
                            <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-2">{{ $wash->wash_code }}</td>
                                <th scope="row"
                                    class="flex items-center px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <img src="{{ asset('upload_img/' . $wash->wash_image) }}" alt=""
                                        class="w-auto h-7 mr-3 rounded-lg">
                                    {{ $wash->wash_name }}
                                </th>
                                <td class="px-4 py-2">{{ $wash->wash_description }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $wash->marked_price }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $wash->offer_percentage }}
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $wash->offer_price }}
                                </td>
                                ...
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div class="flex items-center">
                                        ...
                                        <span class="ml-1 text-gray-500 dark:text-gray-400">{{ $wash->rating }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $wash->wash_duration }}
                                ...
                                <td class="px-4 py-2"><label class="items-center mb-5 cursor-pointer">
                                        <input wire:click="toggle_status({{ $wash->id }})" type="checkbox"
                                            class="sr-only peer" {{ $wash->wash_status == 1 ? 'checked' : '' }}>
                                        <div
                                ...
                                <td class="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <div x-data="{ open: false }" class="relative">
                                        ...
                                        <div x-show="open" @click.away="open = false" x-transition
                                            ...
                                            <a wire:click="delete({{ $wash->id }})"
                                                wire:confirm="Are you sure you want to delete this wash Label?"
                                                class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"><svg
